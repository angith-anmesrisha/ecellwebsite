...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const currentOverrideState =
      localStorage.getItem("ecell_admin_override_unlocked") === "true";
  ...
  const fetchSpreadsheetData = async () => {
    setIsLoading(true);
    try {
      const localRegistry = localStorage.getItem(
        "ecell_submissions_backup_tree",
      );
      if (localRegistry) {
    ...
    if (confirmStep1) {
      ...
      if (confirmStep2) {
        ...
        try {
          ...
          setSubmissions([]);
          alert(
            "Database Cluster Successfully Cleared! All candidate profiles have been purged.",
          );
        } catch (err) {
          alert(
            "Database link timed out during deletion routine. Check your script webhook connection.",
          );
        } finally {
          setIsLoading(false);
        }
  ...
  const filteredData =
    filterDept === "all"
      ? submissions
      : submissions.filter((item) => item.dept === filterDept);
      ...
      <div className="min-h-screen bg-black text-white flex flex-col justify-center items-center px-4 font-sans antialiased">
        <div className="w-full max-w-sm bg-zinc-950 border border-white/10 p-6 rounded-2xl shadow-2xl space-y-4">
          <div className="text-center space-y-1">
            <Lock
              className="text-blue-500 mx-auto mb-2 animate-pulse"
              size={32}
            />
            <h2 className="text-xl font-black tracking-tight">
              Admin Matrix Verification
            </h2>
            <p className="text-xs text-white/40">
              Input encrypted code to mount system analytics.
            </p>
          </div>
          <form onSubmit={handleLogin} className="space-y-3">
            <input
              type="password"
              placeholder="System Master Access Key"
              value={password}
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-white text-black font-bold uppercase text-[10px] tracking-widest rounded-xl hover:bg-zinc-200 transition"
            >
              Verify Credentials
            </button>
          </form>
    ...
    <div className="min-h-screen bg-black text-white py-16 px-4 md:px-8 font-sans antialiased">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/10 pb-6 gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-black tracking-tight mt-1">
              E-Cell Recruitment Workspace
            </h1>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            {/* 🗑️ THE RESET BUTTON GRAPHIC INTERACTION */}
            <button
              onClick={handleWipeLeaderboardDatabase}
              disabled={isLoading}
              className="text-xs font-mono border border-red-500/30 bg-red-500/15 px-4 py-1.5 rounded-lg text-red-400 font-bold hover:bg-red-500/25 transition flex items-center gap-1.5 cursor-pointer disabled:opacity-40"
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="text-xs font-mono border border-white/10 bg-white/5 px-4 py-1.5 rounded-lg text-white font-bold hover:bg-white/10 transition flex items-center gap-1.5 cursor-pointer disabled:opacity-40"
            >
              {isLoading ? "Syncing Workspace..." : "🔄 Refresh Data"}
            </button>
            <button
              onClick={() => (window.location.href = "/recruitment")}
              className="text-xs font-mono border border-white/10 bg-white/5 px-3 py-1.5 rounded-lg text-white/60 hover:text-white transition flex items-center gap-1.5 shrink-0 cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div
            className={`border rounded-2xl p-6 space-y-4 shadow-xl transition-all duration-300 ${
              isPortalForceUnlocked
          ...
          >
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <div className="text-xs font-bold uppercase font-mono tracking-wider text-white/80">
                  Manual System Gate Switch
                </div>
                <p className="text-[11px] text-white/40">
                  Force open or slam shut student portal routes instantly.
                </p>
              </div>
              {isPortalForceUnlocked ? (
            ...
            <button
              onClick={handleTogglePortalGate}
              className={`w-full py-3 rounded-xl font-mono text-[10px] uppercase font-black tracking-widest transition-all duration-300 shadow-lg cursor-pointer ${
                isPortalForceUnlocked
            ...
            >
              {isPortalForceUnlocked
                ? "🚨 Force Lock Recruitment Portal"
                : "🔓 Force Open Recruitment Portal"}
            </button>
          </div>
          ...
          <div className="bg-zinc-950 border border-white/10 rounded-2xl p-6 space-y-4">
            <div className="flex items-center gap-2 text-xs font-bold tracking-wider uppercase font-mono text-white/80">
              <CalendarIcon size={16} className="text-blue-500" /> Automated
              Calendar Clock Setup
            </div>
            <form
              onSubmit={handleUpdateLaunchDate}
            ...
            >
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl p-2.5 text-xs text-white font-mono focus:outline-none focus:border-blue-500 block dark:[color-scheme:dark]"
                />
                <input
                  type="time"
              ...
              <button
                type="submit"
                className={`w-full py-2.5 text-[10px] uppercase tracking-wider font-bold rounded-xl flex items-center justify-center gap-1.5 transition ${
                  isDateSaved
              ...
              >
                <Save size={12} />{" "}
                {isDateSaved ? "Target Sync Locked" : "Commit Date Sequence"}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-zinc-950 border border-white/10 p-4 rounded-xl flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-mono font-black mt-1 block">
                {submissions.filter((s) => s.dept === "ops").length}
              </span>
            </div>
          ...
          <div className="bg-zinc-950 border border-white/10 p-4 rounded-xl flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-mono font-black mt-1 block">
                {submissions.filter((s) => s.dept === "media").length}
              </span>
            </div>
          ...
          <div className="bg-zinc-950 border border-white/10 p-4 rounded-xl flex items-center justify-between">
            <div>
              ...
              <span className="text-xl font-mono font-black mt-1 block">
                {submissions.filter((s) => s.dept === "spons").length}
              </span>
            </div>
        ...
        <div className="flex gap-2 border-b border-white/5 pb-4">
          {["all", "ops", "media", "spons"].map((tab) => (
            <button
              key={tab}
              onClick={() => setFilterDept(tab)}
              className={`px-4 py-2 rounded-xl border text-xs font-mono uppercase font-bold transition cursor-pointer ${
                filterDept === tab
        ...
        <div className="bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden">
          {filteredData.length === 0 ? (
            <div className="p-12 text-center text-xs font-mono text-white/30">
              No candidate records logged inside this sector node loop. Click
              refresh to query Google Sheets.
            </div>
          ) : (
                  ...
                  {filteredData.map((item, idx) => (
                    <React.Fragment key={item.id || idx}>
                      <tr className="border-b border-white/5 hover:bg-white/[0.01] transition group">
                        ...
                        <td className="p-4">
                          <span
                            className={`px-2 py-0.5 border rounded uppercase text-[10px] font-bold ${
                              item.dept === "ops"
                        ...
                        <td className="p-4 text-center">
                          <button
                            ...
                            className="px-2 py-1 bg-white/5 border border-white/10 text-white font-bold rounded hover:bg-white/10 transition text-[10px] uppercase flex items-center gap-1 mx-auto cursor-pointer"
                          >
                            <FileText size={11} /> Inspect Packets{" "}
                            {expandedRow === item.id ? (
                              <ChevronUp size={11} />
                            ) : (
                              <ChevronDown size={11} />
                        ...
                        <tr className="bg-zinc-900/30 border-b border-white/10">
                          <td colSpan={5} className="p-6 space-y-5">
                            <div className="space-y-2">
                              <div className="text-[10px] text-blue-400 font-bold tracking-widest uppercase flex items-center gap-1">
                                <HelpCircle size={12} /> Round 1 MCQ Audit Log
                              </div>
                              {item.round1Choices &&
                              item.round1Choices.length > 0 ? (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px]">
                                  {item.round1Choices.map((choice, cIdx) => (
                                    <div
                                      key={cIdx}
                                      className="bg-black/50 border border-white/5 p-2.5 rounded-lg text-white/70 flex gap-2"
                                    >
                                ...
                                <div className="text-[11px] text-white/30 italic">
                                  // No MCQ breakdown string returned for entry
                                </div>
                              )}
                            </div>
                            <div className="space-y-2">
                              <div className="text-[10px] text-amber-500 font-bold tracking-widest uppercase flex items-center gap-1">
                                <CheckSquare size={12} /> Round 2 Case
                                Resolution Response
                              </div>
                              {item.caseAnswer ? (
                                <div className="bg-black/50 border border-white/5 rounded-xl p-4 text-xs text-white/80 leading-relaxed italic">
                                  "{item.caseAnswer}"
                                </div>
                              ) : (
                                <div className="text-[11px] text-white/30 italic">
                                  // Case Simulation incomplete or dropped
                                </div>
                              )}
                            </div>
                          </td>
