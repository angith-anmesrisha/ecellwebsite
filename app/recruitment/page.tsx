...
export default function RecruitmentPortal() {
  ...
  const quizQuestions = [
    {
      q: "A high-priority project cross-functional team is lagging because two senior vertical managers disagree sharply on resource allocation. What is your intervention framework?",
      options: [
  ...
  useEffect(() => {
    ...
    const checkLockStatus = () => {
      ...
      const emergencyState = localStorage.getItem("ecell_admin_override_unlocked") === "true";
      if (emergencyState) {
        setIsSystemLocked(false);
        return;
      }
    ...
    if (currentQuestionIdx + 1 < quizQuestions.length) {
      ...
      try {
        ...
        if (auditData.success) {
          ...
          // Local fallback backup logic for the admin dashboard query layout
          ...
          alert(auditData.error || "Submission transaction rejected by server.");
        }
      } catch (err) {
        alert("Network handshake timeout. Verify gateway connections and retry.");
      } finally {
  ...
  const handleSubmitRound2Case = async (e: React.FormEvent) => {
    e.preventDefault();
    if (currentWords < 50) {
      alert("Strategic solutions must be substantive. Write at least 50 words.");
      return;
    }

    if (activeCandidate && confirm("Lock final corporate strategy framework? Submissions cannot be modified.")) {
      setIsSubmitting(true);
      try {
        ...
        if (resData.success) {
          ...
          // Re-sync fallback backup array mapping for active reviews
          const localLogTree = JSON.parse(localStorage.getItem("ecell_submissions_backup_tree") || "[]");
          ...
          alert(resData.error || "Failed to commit strategy data node to server backend.");
        }
      } catch (err) {
        alert("Network loss detected. Please re-submit your response draft.");
      } finally {
        setIsSubmitting(false);
      }
    }
        ...
        <div className="w-full max-w-2xl text-center space-y-10">
          <div className="flex justify-center">
            <div className="p-4 bg-zinc-950 border border-white/5 rounded-full shadow-inner">
              <Clock size={32} className="text-blue-500 animate-spin" style={{ animationDuration: "12s" }} />
            </div>
          </div>
          <h1 className="text-2xl md:text-4xl font-black uppercase tracking-wider text-white max-w-xl mx-auto leading-tight">EXECUTIVE RECRUITMENT IS LOCKED</h1>
          <div className="flex justify-center items-center gap-3 md:gap-4 max-w-lg mx-auto">
            {[
              ...
              { label: "MINS", val: minutesRemaining },
              { label: "SECS", val: secondsRemaining }
            ].map((unit, i) => (
          ...
          <div className="pt-4 max-w-xs mx-auto">
            <div className="text-[9px] font-mono text-white/5 uppercase tracking-widest mb-1 select-none">[ COMMAND OVERRIDE ]</div>
            <input type="password" placeholder="System Override Passkey" value={bypassInput} onChange={handleBypassCheck} className="w-full text-center bg-transparent border-none text-xs font-mono tracking-widest text-white/10 placeholder-transparent focus:outline-none focus:text-blue-500/40 transition-colors" autoComplete="off" />
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2">
          <div className="text-[10px] uppercase font-mono tracking-widest bg-blue-500/10 text-blue-400 px-3 py-1 rounded-full border border-blue-500/20 inline-block font-black">
            BIMTECH E-CELL OFFICIAL MANAGEMENT COHORT ASSESSMENT {isBypassed && <span className="text-red-400 ml-1">// BYPASS ACTIVE</span>}
          </div>
          <h1 className="text-3xl font-black uppercase tracking-tight">Recruitment Staging Funnel</h1>
        </div>

        {!activeCandidate ? (
          <div className="bg-zinc-950 border border-white/10 rounded-2xl p-6 md:p-8 max-w-md mx-auto space-y-6 shadow-2xl">
            <div className="space-y-1 text-center font-mono">
              <UserPlus className="text-blue-500 mx-auto mb-1 animate-pulse" size={28} />
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Initialize Application Profile</h3>
              <p className="text-[11px] text-white/40">Input corporate configuration details to open evaluation matrices.</p>
            </div>
            <form onSubmit={handleRegisterCandidate} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-mono text-white/40 tracking-wider">Candidate Name</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-mono text-white/40 tracking-wider">Target Department Track</label>
              ...
              <button type="submit" className="w-full py-2.5 bg-white text-black font-mono font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-zinc-200 transition">Mount Assessment Staging</button>
            </form>
          </div>
        ) : (
          <>
            <div className="flex justify-center items-center gap-4 border-b border-white/5 pb-4 font-mono text-[10px] tracking-widest uppercase font-bold select-none">
              ...
              <div className={`flex items-center gap-1.5 ${currentRound === 2 ? "text-amber-500" : "text-white/20"}`}>
                ...
                {round1Passed ? "Crisis Simulation Unlocked" : "Crisis Simulation Gate Locked"}
              ...
              {currentRound === 1 && !quizFinished ? (
                <motion.div key="round1-quiz" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                  <div className="p-5 border-b border-white/10 bg-white/[0.01] flex justify-between items-center font-mono text-[10px] text-white/40 uppercase tracking-widest">
                    <span>Strategic Evaluation Case Block</span>
                    <span className="font-bold text-white">Item {currentQuestionIdx + 1} of {quizQuestions.length}</span>
                  </div>
                ...
                <motion.div key="round1-failed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-zinc-950 border border-red-500/20 rounded-2xl p-8 text-center space-y-4 max-w-md mx-auto font-mono">
                  <AlertTriangle className="text-red-500 mx-auto animate-bounce" size={36} />
                  <div className="space-y-1">
                    <h3 className="text-white font-bold text-sm uppercase tracking-wider">Gate Review Halt</h3>
                    <p className="text-xs text-white/40">Evaluation index score complete: {runningScore !== null ? `${runningScore}/100` : "CALCULATING..."}. Target metrics require a 40 point boundary threshold to unmount scenario locks.</p>
                  </div>
                </motion.div>
              ) : (
                <motion.div key="round2-case" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full bg-zinc-950 border border-white/10 rounded-2xl overflow-hidden flex flex-col shadow-2xl">
                  <div className="p-5 border-b border-white/10 bg-white/[0.01] flex items-center justify-between">
                    ...
                    <div className="text-[10px] font-mono uppercase bg-blue-500/10 border border-blue-500/20 text-blue-400 px-2 py-0.5 rounded">Track: {activeCandidate.dept.toUpperCase()} Vertical Matrix</div>
                  </div>

                  <div className="p-6 space-y-5">
                    <div className="bg-amber-500/5 border border-amber-500/10 p-4 rounded-xl space-y-2">
                      <div className="flex items-center gap-1.5 font-mono text-[10px] font-black tracking-widest uppercase text-amber-400"><Clock size={12} /> Live Corporate Prompt Manifest (Deterministic)</div>
                      ...
                      <form onSubmit={handleSubmitRound2Case} className="space-y-3">
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-[10px] uppercase font-mono text-white/40 tracking-wider">
                            <label className="font-bold flex items-center gap-1"><Cpu size={12} /> Corporate Resolution Response Plan</label>
                            <span className={currentWords < 50 ? "text-amber-500" : "text-emerald-400"}>{currentWords} / Min 50 Words</span>
                          </div>
                          <textarea disabled={isSubmitting} required rows={6} value={userR2Submission} onChange={(e) => setUserR2Submission(e.target.value)} placeholder="Outline your immediate turnaround sequence, stakeholder messaging, strategic resource staging pipelines, and long-term risk mitigation models..." className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-xs text-white placeholder-white/20 focus:outline-none focus:border-amber-500 font-mono resize-none leading-relaxed disabled:opacity-50" />
                        </div>
                        <button type="submit" disabled={currentWords < 50 || isSubmitting} className="w-full py-2.5 bg-white text-black text-[10px] font-mono tracking-widest font-black uppercase rounded-xl flex items-center justify-center gap-2 transition hover:bg-zinc-200 disabled:opacity-20 cursor-pointer"><Send size={12} /> Commit Strategic Solution Node</button>
                      </form>
                    ) : (
                      <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-5 text-center space-y-2 font-mono">
                        <CheckCircle2 className="text-emerald-500 mx-auto animate-pulse" size={32} />
                        <h4 className="text-white text-xs uppercase font-bold tracking-wider">Strategic Framework Sealed</h4>
                        <p className="text-[11px] text-white/40 max-w-sm mx-auto">Your operational response portfolio has been successfully logged to the master executive admin dashboard panels.</p>
                      </div>
                    )}
                  </div>
                </motion.div>
        ...
        <div className="pt-8 text-center font-mono text-[9px] text-white/5 hover:text-white/20 transition-colors select-none">
          <button type="button" onClick={() => window.location.href = "/recruitment/admin"} className="hover:underline cursor-pointer">// Master Core Telemetry Console Loop //</button>
        </div>
